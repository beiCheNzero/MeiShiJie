<?php
  ...
  if($search != ''){
    $se = "SELECT * FROM food_product WHERE food_name LIKE '%{$_SESSION["search"]}%'";
    $se2 = mysqli_query($conn,$se);
    $se3 = "SELECT COUNT(food_name) FROM (SELECT * FROM food_product WHERE food_name LIKE '%{$search}%') p";
    $result = mysqli_query($conn,$se3);
    $se5 = mysqli_fetch_array($result);
    // $_SESSION['result'] = $se5;
    $count=$se5['COUNT(food_name)'];
    $true = true;
    // echo $se5[0];
    if($se5[0] == ""){
      echo '<html>
          ...
          <title>提示</title>
            <script>alert("没有找到相关菜谱，请重新搜索！");</script>
          </head>
          </html>';
    }
  }else{
    echo '<html>
          ...
          <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
          <title>提示</title>
            <script>alert("请输入关键词！");</script>
          </head>
          </html>';
    header('Location: menu.php');//成功则跳转登陆界面
    // $d="select * from food_product";
    // $d2=mysqli_query($conn,$d);
    // $d3="select count(food_name) from food_product";
    // $result=mysqli_query($conn,$d3);
    // $d5=mysqli_fetch_array($result);
    // $count=$d5['count(food_name)'];
    // $true = false;
    }
  ...
  <div class="header">
    <div class="header_con">
      ...
      <div class="header_con_main">
        <form action="./menu_search.php" method="POST" class="search" >
        ...
        <div class="liststyle_q">
          <div class="liststyle_list">
            ...
            <?php
              ...
              $sql1 = "select * from (SELECT * FROM food_product WHERE food_name LIKE '%{$search}%') p limit ".($page-1) * 12 .",12";//查询语句，limit后的两个参数第一个是查询的起始位置，第二个是显示的数据条数
              $result=mysqli_query($conn,$sql1);

              
              if($true == true){
                for($i = 0;$i<$count;$i++){
                  $rs=mysqli_fetch_array($se2);
              ...
              $to_sql="SELECT COUNT(*) FROM (SELECT * FROM food_product WHERE food_name LIKE '%{$search}%') p";
